...
def es1(objective_function, chromosome_length, population_size, number_of_iterations,
       number_of_offspring, number_of_parents, sigma, tau, tau_0,
       log_frequency=1, cut=False, mutation='multidim', **kwargs):
    diagnostic_data = {}
    best_solution = np.empty((1, chromosome_length))
    ...
    if mutation == 'p3':
#         print(sigma[:,0].shape)
#         print(np.ones((population_size, chromosome_length)).shape)
        current_population_sigmas = sigma[:,0] * \
            np.ones((population_size, chromosome_length))
    else:
        current_population_sigmas = sigma * \
            np.ones((population_size, chromosome_length))
